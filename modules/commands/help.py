    ...
    async def apply(self):
        ...
        **biene jointeam [teamname] [mention all the users you want to add to the team]**
        ...
        if get(self.author.roles, name='Organizer') is None:
            ...
            description += """**biene createteam [teamname] [mention 1 member of new team]** -> Creates new team
            **biene userinfo [mention the user you want to stalk :smirk:]** -> Get information of a user
            **biene reactions** -> display a list of reactions by username and emoji from a message
            **biene addsticker [sticker name] [mention the username that receives the sticker]** -> Give sticker
            """
